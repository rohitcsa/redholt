		...
		var _this = {
			...
			drawing: {
				...
				handler: {
					...
					change: function( options ) {
						var state, i1;
						var current = _this.drawing.buffer.selection;
						...
						if ( options.width ) {
							...
							if ( current instanceof fabric.Text ) {
								current.fontSize = options.width * 3;
							} else {
								_this.setup.fabric.freeDrawingBrush.width = options.width;
							}
						}

						if ( options.color ) {
							_this.drawing.color = options.color;
							_this.setup.fabric.freeDrawingBrush.color = options.color;
						}

						if ( current ) {
							if ( current instanceof fabric.Group ) {
								_this.setup.fabric.remove( current );
								_this.drawing.handler.line( _this.deepMerge( current.cfg, {
									color: options.color || _this.drawing.color,
									width: options.width || _this.drawing.width,
									group: []
								}, true ) );
								return;
							}
							...
							// ADD UNDO
							state = JSON.stringify( current.saveState().originalState );
					...
					text: function( options ) {
						var cfg = _this.deepMerge( {
							...
							top: _this.setup.fabric.height / 2,
							left: _this.setup.fabric.width / 2,
							fontSize: _this.setup.chart.fontSize,
							fontFamily: _this.setup.chart.fontFamily,
							fill: _this.drawing.color
						}, options || {} );
			...
			capture: function( options, callback ) {
				...
				_this.setup.fabric = new fabric.Canvas( _this.setup.canvas, _this.deepMerge( {
					...
					isDrawingMode: cfg.drawing.mode == "pencil"
				}, cfg ) );
				...
				_this.drawing.mode = cfg.drawing.mode;
				_this.drawing.arrow = cfg.drawing.arrow;

				// RELIABLE VARIABLEs
				_this.deepMerge( _this.drawing, cfg.drawing );

				// OBSERVE MOUSE EVEMTS
				_this.setup.fabric.on( "mouse:over", function( e ) {
					_this.drawing.buffer.isSelectable = e.target.selectable;
					_this.setup.fabric.isDrawingMode = _this.drawing.mode == "pencil" ? !_this.drawing.buffer.isDrawing ? !e.target.selectable : true : false;
				} );
				_this.setup.fabric.on( "mouse:down", function( e ) {
					_this.drawing.buffer.isDrawing = _this.setup.fabric.isDrawingMode;
					if ( !_this.drawing.buffer.isSelected && !_this.drawing.buffer.isSelectable && _this.drawing.mode != "pencil" ) {
				...
				_this.setup.fabric.on( "selection:cleared", function( e ) {
					...
					_this.drawing.buffer.selection = false;
					...
					setTimeout( function() {
						...
						if ( _this.drawing.buffer.isDrawingMode ) {
				...
				_this.setup.fabric.on( "object:added", function( e ) {
					...
					if ( item.selectable && !item.known && !item.noUndo ) {
						...
						_this.drawing.redos = [];
						_this.setup.fabric.setActiveObject( item );
					}
